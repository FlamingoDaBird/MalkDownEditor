...
- **Status:** RESOLVED (2026-05-23)
- **Severity:** Major
- **Description:** When a pasted/uploaded image created a local attachment and Ctrl+Z removed that newly inserted image, MD Editor could leave the saved attachment file on disk without asking whether to delete it.
- **Fix:** Freshly saved uploads are tracked until the Markdown document references them. If a later edit or the delayed upload check shows the upload was abandoned or undone, MD Editor now shows the same delete-from-disk prompt and uses the undo-safe pending deletion flow. Confirmed deletions also flush after a short grace period when there is no next edit.
...
- **Status:** RESOLVED (2026-05-23)
- **Severity:** Major
- **Description:** User still sees `Loading editor...` even though MD Editor output shows `editor.create()` resolved, `loadingCount:0`, and visible `.milkdown`/`.ProseMirror` DOM nodes. This suggests a stale build, inactive/hidden webview, or visual overlay/tab-state mismatch rather than Milkdown startup itself.
